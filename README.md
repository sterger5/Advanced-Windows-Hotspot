...
- 自动检测上行网卡（Wi-Fi / 以太网），通过 **COM INetSharingManager** API 自动配置 ICS
...
4. **允许联网** — 开启后连接热点的设备可通过本机上网
5. **使用系统热点** — 切换至 Windows 原生移动热点模式（支持频段选择）
6. **点击启动/停止** — 开始或停止热点

### 提示 | Tips

- **WiFi Direct 模式**（默认）：无需网络连接即可创建热点，适合无网络环境使用。设备连接后自动配置 ICS 上网。如自动配置失败，可点击「启用网络共享」手动配置。
- **系统热点模式**：通过 Windows 移动热点创建，支持频段选择，需要网络连接。
